...
class AdoptionApplicationViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        method='get',
        responses={200: AdoptionReviewSerializer(many=True)},
    )
    @swagger_auto_schema(
        method='post',
        request_body=AdoptionReviewSerializer,
        responses={201: AdoptionReviewSerializer},
    )
    ...
    @swagger_auto_schema(
        method='get',
        responses={200: AdoptionChatMessageSerializer(many=True)},
    )
    @swagger_auto_schema(
        method='post',
        request_body=AdoptionChatMessageSerializer,
        responses={201: AdoptionChatMessageSerializer},
    )
    @action(detail=True, methods=['get', 'post'])
