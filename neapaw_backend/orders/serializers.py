...
class OrderSerializer(serializers.ModelSerializer):
    ...
    hostel_pet_name = serializers.SerializerMethodField()
    hostel_room_name = serializers.SerializerMethodField()
    hostel_check_in_date = serializers.SerializerMethodField()
    hostel_check_out_date = serializers.SerializerMethodField()
    hostel_duration_days = serializers.SerializerMethodField()
    hostel_service_type = serializers.SerializerMethodField()
    hostel_additional_treatments = serializers.SerializerMethodField()
    ...
    def get_hostel_pet_name(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return getattr(getattr(linked_booking, 'pet', None), 'name', None)

    def get_hostel_room_name(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return getattr(getattr(linked_booking, 'room', None), 'name', None)

    def get_hostel_check_in_date(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return linked_booking.check_in_date if linked_booking else None

    def get_hostel_check_out_date(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return linked_booking.check_out_date if linked_booking else None

    def get_hostel_duration_days(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return linked_booking.duration_days if linked_booking else None

    def get_hostel_service_type(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        return linked_booking.get_service_type_display() if linked_booking else None

    def get_hostel_additional_treatments(self, obj):
        linked_booking = getattr(obj, 'linked_hostel_booking', None)
        if not linked_booking:
            return []
        return list(linked_booking.additional_treatments.values_list('name', flat=True))
    ...
    def update(self, instance, validated_data):
        ...
        if shipping_address_data:
            if instance.shipping_address_id:
                for field, value in shipping_address_data.items():
                    setattr(instance.shipping_address, field, value)
